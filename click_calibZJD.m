...
function click_calibZJD(calib_data)
...
if isempty(calib_data.I{calib_data.ind_active(1)}),
    ...
    if isempty(calib_data.ind_read),
        disp('Cannot extract feature without images');
        return;
    end;
end;
...
%no
% if ~exist('xc_default'),
%     xc_default = round(calib_data.ocam_model.width/2);
% end;
% 
% if ~exist('yc_default'),
%     yc_default = round(calib_data.ocam_model.height/2);
% end;
...
% end
%yes
% calib_data.minMatchedPoints =max(15, 20 / 1000 * calib_data.ocam_model.width); 
% calib_data.maxInitReprojectError = 20 / 1200 * calib_data.ocam_model.width; 
% calib_data.maxFinalReprojectError = 1.5; 
% calib_data.maxInlierError = 1; 
% calib_data.maxSmoothError = 80 / 1000 * calib_data.ocam_model.width; 
%no
...
patternKeyPoints = detectSURFFeatures(calib_data.pattern, 'NumOctaves', 3, 'NumScaleLevels', 3); 
[calib_data.stanardpatternFeatures,calib_data.stanardspatternPoints] = extractFeatures(calib_data.pattern, patternKeyPoints); 
...
if use_video_mode == 0
    ...
    for kk = ima_numbers
        ...
        [callBack]=get_featureboard_features_ZJD(kk,use_corner_find,calib_data);
        if callBack == 1
            count = count + 1;
