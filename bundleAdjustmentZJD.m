...
function bundleAdjustmentZJD(calib_data, robust)
...
% [x0, ~, vExtr, ~, ~, ~, jacExtr] = lsqnonlin(@bundleErrZJD,double(x0), ...
%                                              -inf*ones(1,length(x0),1), inf*ones(1,length(x0)), ...
%                                              optionsLM, calib_data,robust);

[x0, ~, vExtr, ~, ~, ~, jacExtr] = lsqnonlin(@bundleErrZJD,double(x0), ...
                                            [],[], ...
                                             optionsLM, calib_data,robust);

%  [x0, ~, error] = lsqnonlin(@bundleErrZJD,double(x0), ...
%                                              -inf*ones(1,length(x0),1), inf*ones(1,length(x0)), ...
%                                              optionsLM, calib_data,robust);                    
%display(['............!!!!!!!!!!!!!!!!!.........error=',num2str(mean(sqrt(sum(reshape(vExtr,2,[]).^2))))]);                                
...
ss
%display(['............!!!!!!!!!!!!!!!!!.........error=',num2str(mean(sqrt(sum(reshape(v,2,[]).^2))))]);   
rms = sqrt(sum(v.^2)/length(v));
% aveError=sum(v)/length(v);
% mse=sqrt(sum((v-aveError).^2)/length(v));
a=sqrt(sum(reshape(v,2,[]).^2));
b=mean(a);
mse=sqrt(sum((a-b).^2)/length(a));
fprintf(1,'Root mean square[pixel]:  %f\n',rms);
fprintf(1,'Mean square error[pixel]:  %f\n',mse);
calib_data.rms = rms;
