...
function [RRfin,ss]=calibrateZJD(calib_data)
% Yp=Yp_abs-yc;
% Xp=Xp_abs-xc;
...
for kk = calib_data.ima_proc,
   ...
   % A=[Yt.*Xpt, Xt.*Xpt, -Yt.*Ypt, -Xt.*Ypt, Xpt, -Ypt];
    %    [V,D]=eig(A'*A);
    [U,S,V] = svd(A);
...
%       Ypt=Yp(:,:,i);
    ...
    if(license('checkout','Optimization_Toolbox') ~= 1)
        ...
        diff_order = 2;
        mono_rho = [];
       % mmm=calib_data.photosInfo().photoPoints(:,2)
        tempmax=0;
       ...
       % [s_o,ob_resnorm,ob_residual,ob_exitflag,ob_output,ob_lambda] = lsqlin(PP,QQ,[],[],[],[],[],[],[],options);
        warning('on','all');
