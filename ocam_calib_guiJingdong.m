...
function ocam_calib_guiJingdong

cell_list = {};

fig_number = 1;
title_figure = 'Improved_Feature_Descriptor Calibration Toolbox by jingdongzhang';
cell_list{1,1} = {'Load Pattern','pattern_load(calib_data)'}
cell_list{1,2} = {'Read names','data_calib(calib_data);'};
cell_list{1,3} = {'Extract feature','click_calibZJD(calib_data);'};
cell_list{2,1} = {'Calibration','calibrationZJD(calib_data);'};

cell_list{2,2} = {'Non-linear Refinement','bundleAdjustmentZJD(calib_data,false);'};
%cell_list{2,2} = {'Robust Non-linear Refinement','bundleAdjustmentZJD(calib_data,true);'};
%cell_list{2,3} = {'Reproject on images','reproject_calib(calib_data);'};
%cell_list{2,4} = {'Show Extrinsic','create_simulation_points(calib_data);'};

%cell_list{3,1} = {'Analyse error','analyse_error(calib_data);'};
%cell_list{3,2} = {'Recomp. corners','recomp_corner_calib(calib_data);'};
%cell_list{3,3} = {'Show calib results','show_calib_results(calib_data);'};
cell_list{2,3} = {'Save','saving_calib(calib_data);'};

cell_list{3,1} = {'Load','loading_calib;'};
cell_list{3,2} = {'Export Data','exportData2TXT(calib_data);'};
cell_list{3,3} = {'Exit',['clear; disp(''Bye. To run again, type ocam_calib.''); close(' num2str(fig_number) ');']}; %{'Exit','calib_gui;'};
