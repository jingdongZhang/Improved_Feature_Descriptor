...
function [callBack] = get_featureboard_features_ZJD(kk, use_corner_find, calib_data)
...
if (size(I, 3) > 1) 
 ...
 photo1 = I; 
 photo2 = histeq(I);
 calib_data.photosInfo(end+1).photo = I; 
 photoIndex = kk; 
...
patternFeatures =calib_data.stanardpatternFeatures; 
patternPoints = calib_data.stanardspatternPoints; 
...
if (size(indexPairs, 1) < max(calib_data.minMatchedPoints,8))
    %obj.photosInfo(end).valid = false; 
    valid = false; 
    calib_data.photosInfo(end).valid=false;
    callBack=0;
...
[~, inliersMaskF] = estimateFundamentalMatrix(patternPoints, photoPoints, 'Method', 'RANSAC', ...
                                    'DistanceThreshold',calib_data.maxInlierError);
estimator = vision.GeometricTransformEstimator;
estimator.Transform = 'projective';
estimator.AlgebraicDistanceThreshold = calib_data.maxInlierError;
...
if (size(patternPoints, 1) < max(calib_data.minMatchedPoints, 4))
    %obj.photosInfo(end).valid = false; 
    calib_data.photosInfo(end).valid=false;
    valid = false; 
    callBack=0;
...
calib_data.photosInfo(end).valid = true; 
calib_data.photosInfo(end).patternPoints = patternPoints; 
calib_data.photosInfo(end).photoPoints = photoPoints; 
calib_data.photosInfo(end).nPoints = size(patternPoints, 1); 
valid = true; 
callBack=1;
...
%are still empty
% if isempty(min_i)
%     min_i = startingCorner(1);%startposition x
%     min_j = startingCorner(2);%startposition 
% end
